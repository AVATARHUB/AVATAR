 ...
 */
public class ClusterCreate extends AsyncTask<String, Void, ArrayList<ClusterMarker>> {
	 ...
	 */
	public ClusterCreate(ArrayList<MarkerPlus> points, MapViewer map, Projection projection, boolean shouldClear){
		this.points = points;
		this.map = map;
		this.bounds = bounds;
	 ...
	 */
	public double pixelDistance(ClusterMarker c1, ClusterMarker c2, Projection projection){
	 ...
	 */
	public ClusterMarker mergegroupClusters(ClusterMarker c1, ClusterMarker c2){
		
		ClusterMarker cluster = new ClusterMarker();
	 ...
	 */
	public ArrayList<ClusterMarker> doInBackground(String... args){
		
		double pixelDistance;
		ArrayList<ClusterMarker> groupClusters = new ArrayList<ClusterMarker>();
		ArrayList<ClusterMarker> allClusters = new ArrayList<ClusterMarker>();
		...
		for(int i = 0; i < tempPoints.size(); i++){
			...
			if(!tempPoint.getName().equals("EMERGENCY")){
				groupClusters.add(new ClusterMarker());
				groupClusters.get(groupClusters.size()-1).addPoint(tempPoint);
			}
			else{
				allClusters.add(new ClusterMarker());
...
//					System.out.println("Point Names 2: " + groupClusters.get(j).getPointNames());
	 ...
	 */
	protected void onPostExecute(ArrayList<ClusterMarker> clusters){
